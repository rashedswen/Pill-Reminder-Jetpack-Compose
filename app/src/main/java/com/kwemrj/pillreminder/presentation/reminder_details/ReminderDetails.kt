...
import com.kwemrj.pillreminder.presentation.reminder_details.component.PillCount
        ...
        Column(
            ...
            Column(
                ...
                Row(
                    ...
                    PillCount(text = "taken", value = state.takenPills)
                    PillCount(text = "Inventory", value = state.inventoryPills)
                    PillCount(text = "missed", value = state.missedPills)
                    PillCount(text = "skipped", value = state.skippedPills)
                ...
                Column(
                    ...
                    Row(
                        ...
                        if (state.lastThreePills.isNotEmpty()) {
                            LazyColumn() {
                                items(state.lastThreePills) {
                                    Text(
                                        text = SimpleDateFormat(
                                            "EEE, MMM d HH a",
                                            Locale.getDefault()
                                        ).format(it.time) + ", 1 pill(s) taken"
                                    )
                                    Spacer(modifier = Modifier.size(4.dp))
                                }
                            }
...
@Composable
fun PieChartComp(missedPills: Int, skippedPills: Int, takenPills: Int) {

    val getPieChartData = listOf(
        PieChartData(statusName = "Missed", value = missedPills),
        PieChartData(statusName = "Skipped", value = skippedPills),
        PieChartData(statusName = "Taken", value = takenPills),
    )
